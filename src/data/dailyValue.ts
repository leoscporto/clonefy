...
import { formatDate2, getEndOfDay, getStartOfDay, isSameDay } from "@utils/dateProcessing";
...
export interface DailyValue {
  ...
  export function extractValues(dailyValues: DailyValue[], dates: Date[]): [number[], GraphDetails[]] {
    const dataset : GraphDetails[] = []
    const values : number[] = []
    for (let i = 0 ; i < dailyValues.length; i++) {


      dataset.push({x: dates[i], y: dailyValues[i].value, z: dailyValues[i].numSells})
  ...
  export async function calculateDailyValues(product: string, transactions: Transaction[], startDate: Date, endDate: Date | null): Promise<[number, DailyValue[]]> {
    ...
    while (currentDate <= end) {
      ...
      const transactionsOnDate = transactions.filter(transaction => {
        ...
        if (sameDay) {
          return transaction.transaction_date.getHours() === currentDate.getHours()
        }
      ...
      const existSellOnThisDate = dateVendasRecords.find((record) => 
         record.date === formatDate2(currentDate,sameDay)
      )
